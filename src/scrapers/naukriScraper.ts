...
export async function scrapeNaukri(query: string, location?: string): Promise<DetailedJob[]> {
  ...
  try {
    ...
    const listings = await page.evaluate(() => {
      ...
      for (const card of cards.slice(0, 50)) {
        try {
          ...
          let titleA: HTMLAnchorElement | null = null;
          for (const selector of titleSelectors) {
            titleA = card.querySelector(selector) as HTMLAnchorElement | null;
            if (titleA && titleA.href) break;
          }
          ...
          for (const selector of companySelectors) {
            companyEl = card.querySelector(selector) as HTMLElement | null;
            if (companyEl && companyEl.textContent) break;
          }
          ...
          let locEl: HTMLElement | null = null;
          for (const selector of locSelectors) {
            locEl = card.querySelector(selector) as HTMLElement | null;
            if (locEl && locEl.textContent) break;
          }
    ...
    // Limit results to SCRAPE_LIMIT
    const limitedListings = (listings as any[]).slice(0, CONFIG.SCRAPE_LIMIT);
    for (const item of limitedListings) {
      try {
        ...
        const enriched = await jobPage.evaluate((base) => {
          ...
          for (const selector of descSelectors) {
            const descSel = document.querySelector(selector) as HTMLElement | null;
          ...
          for (const selector of applySelectors) {
            const apply = document.querySelector(selector) as HTMLAnchorElement | null;
            if (apply && apply.href) {
