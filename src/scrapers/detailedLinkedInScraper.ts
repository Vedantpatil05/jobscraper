...
import { DetailedJob, ScrapeOptions } from "../types";

puppeteer.use(StealthPlugin());

export async function scrapeLinkedInDetailed(query: string, location?: string, geoId?: string, options: ScrapeOptions = {}): Promise<DetailedJob[]> {
  ...
  try {
    ...
    // Build search URL for jobs posted in configured time range
    const encoded = encodeURIComponent(query);
    const locationParam = location ? `&location=${encodeURIComponent(location)}` : "";
    const geoParam = geoId ? `&geoId=${encodeURIComponent(geoId)}` : "";
    const timeRangeSeconds = CONFIG.TIME_FILTER_DAYS * 24 * 60 * 60; // Convert days to seconds
    const searchUrl = `https://www.linkedin.com/jobs/search?keywords=${encoded}&f_TPR=r${timeRangeSeconds}${locationParam}${geoParam}`;
    ...
    const jobUrls = await page.evaluate((limit: number) => {
      ...
      for (const containerSel of containerSelectors) {
        ...
        if (container) {
          const found = Array.from(container.querySelectorAll("a[href*='/jobs/view/']")) as HTMLAnchorElement[];
          anchors = found;
      ...
      if (anchors.length === 0) {
        anchors = Array.from(document.querySelectorAll("a[href*='/jobs/view/']")) as HTMLAnchorElement[];
      }
    ...
    }, Math.max(1, Math.min(CONFIG.SCRAPE_LIMIT || 10, 50)) as any);
    ...
    const variantTokens = new Set<string>(queryTokens);
